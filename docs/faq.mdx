...
`failedValidation`, exactly when the user needs to retry. See [Form-level state](./form-state/index.mdx).
...
it at least once. Under `manual` the mismatch shows up on submit instead. If the dependent field was prefilled and the
user never touched it, `markInteracted()` on that field makes it count as edited, and the subscription reaches it from
then on. Also check that the *value* of a watched field changed — a status-only change never fires it. See
[Cross-field logic](./validation/cross-field.mdx).
...
to a banner, and check `field.lastFailure` for the exception. See [Async validation](./validation/async-failures.mdx).
...
state-model change; see [Under the hood](./internals/decisions.mdx#deliberate-residue).
...
over `String?` like the built-ins. See [Validators](./validation/validators.mdx#combining-validators).
...
[Rendering fields](./rendering/index.mdx).
...
data is in. See [Lifecycle and ownership](./lifecycle/prefilled-forms.mdx).
...
that screen. See [Lifecycle and ownership](./lifecycle/index.mdx).
