...
- It obeys the [gate](./modes.mdx): nothing happens while this field is in `ValidationMode.manual`, and nothing on a
  field the user has never edited. Under `manual` the mismatch shows up on submit instead. A prefilled field that
  should react to its dependencies anyway — an instructor loaded from the server whose eligibility depends on the
  aircraft — gets `markInteracted()` once, and from then on the subscription reaches it.
