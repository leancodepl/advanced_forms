...
2. **A field the user has never edited validates nothing on its own**, in every mode. `validate()` is what checks
   those, so a prefilled form does not greet the user with errors. `field.markInteracted()` is the deliberate way
   out: it makes one field count as edited without a value change, for the prefilled field that should start reacting
   to its [dependencies](./cross-field.mdx).
